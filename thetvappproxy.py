...
PORT          = int(os.environ.get("PORT", "8087"))  # Railway define PORT en el entorno[web:117]
...
def index():
    ...
    # Base dinámico según host real (local o Railway)
    base = request.host_url.rstrip("/")  # ej: https://tvchannels-production.up.railway.app
    ...
    for i, ch in enumerate(sorted_channels, 1):
        if ch.get("custom"):
            ...
            stream_link = f"{base}/stream/{ch['slug']}"
        logo      = ch.get("logo", "")
...
def playlist():
    ...
    for ch in all_channels:
        ...
        base_num  = group_base[group]
        counters[group] += 1
        chno      = base_num + counters[group]

        if ch.get("custom"):
            stream_url = ch["stream_url"]
        else:
            stream_url = f"{base}/stream/{ch['slug']}"
